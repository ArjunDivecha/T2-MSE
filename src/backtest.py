...
def run_backtest_sequential(data_dir: str = 'data/', 
    ...
    if save_forecasts and monthly_forecasts:
        try:
            output_file = os.path.join(OUTPUT_DIR, 'T60_Enhanced_MSE.xlsx')
...
def save_backtest_results(results_df: pd.DataFrame, analysis: Dict, 
    ...
    results_path = os.path.join(output_dir, 'backtest_MSE_results.csv')
    results_df.to_csv(results_path, index=False)
    
    # Save analysis summary
    analysis_path = os.path.join(output_dir, 'backtest_MSE_analysis.csv')
