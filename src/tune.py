...
def evaluate_hyperparameters(
    params: Dict[str, Any], train_data: List[Tuple], device: torch.device
) -> Dict[str, float]:
    """
...
def run_hyperparameter_search(
    data_dir: str = "data/", n_months: int = 30, save_results: bool = True
) -> pd.DataFrame:
    """
    ...
    logger.info(
        f"Loaded data: {len(dates)} months, {len(factor_names)} factors"
    )
    ...
    logger.info(
        f"Selecting {n_months} random months for hyperparameter tuning..."
    )
    selected_windows = select_random_months(
        all_windows, n_months=n_months, seed=42
    )
    ...
    for i, params in enumerate(param_combinations):
        logger.info(
            f"Progress: {i+1}/{len(param_combinations)} - Testing: {params}"
        )

        try:
            # Evaluate this parameter combination
            metrics = evaluate_hyperparameters(
                params, selected_windows, device
            )
    ...
    if save_results:
        ...
        results_path = os.path.join(OUTPUT_DIR, "hyperparam_MSE_results.csv")
...
def update_default_config(
    best_params: Dict[str, Any], config_file: str = "src/config.py"
):
    """
    ...
    new_config_section = f"""# Model defaults (updated from tuning)
DEFAULT_CONFIG = {{
    ...
    logger.info(
        f"Updated {config_file} with best hyperparameters"
    )
...
def main():
    ...
    try:
        ...
        results_df = run_hyperparameter_search(
            data_dir="data/", n_months=30, save_results=True
        )
