...
def mse_loss(predictions: torch.Tensor, actual_returns: torch.Tensor) -> torch.Tensor:
    """
    Mean Squared Error loss function for all factors.
    
    Trains the model to predict actual returns as accurately as possible
    for all factors, not just the top 5.
    ...
    Returns:
        loss: Mean squared error between predictions and actual returns
    """
    # Standard MSE loss across all factors
    loss = F.mse_loss(predictions, actual_returns)
    return loss

def mse_loss_eval(predictions: torch.Tensor, actual_returns: torch.Tensor) -> torch.Tensor:
    """
    Evaluation version of MSE loss (same as training version).
    ...
    Returns:
        loss: Mean squared error between predictions and actual returns
    """
    # MSE loss is the same for training and evaluation
    return mse_loss(predictions, actual_returns)
